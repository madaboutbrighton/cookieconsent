...
if (file_exists('../../secret.config.php')) require_once '../../secret.config.php';
...
if (!XPDO_CLI_MODE) {echo '<pre>';} /* used for nice formatting of log messages */
...
$vehicle->resolve('file', array('source' => $sources['source_assets'],
                                'target' => "return MODX_ASSETS_PATH . 'components/';",
                                ));
                                
$vehicle->resolve('file', array('source' => $sources['source_core'],
...
$find = array('#NAME#', '#NAME_LOWER#', '#VERSION#', '#YEAR#');
$replace = array(PKG_NAME, PKG_NAME_LOWER, PKG_VERSION . ' ' . PKG_RELEASE, strftime('%Y'));

$builder->setPackageAttributes(array( 'license' => getSetupFile($sources['docs'] . 'license.txt', $find, $replace),
                                      'readme' => getSetupFile($sources['docs'] . 'readme.txt', $find, $replace),
                                      'changelog' => getSetupFile($sources['docs'] . 'changelog.txt', $find, $replace),
                                      'setup-options' => array('source' => $sources['build'].'setup.options.php'),
                                    )); 

$modx->log(modX::LOG_LEVEL_INFO, 'Added package attributes and set-up options.');
